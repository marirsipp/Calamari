function[Life_method_BV] = SpectralFatigue_Mthd8(path_itr, path_str, Metocean_Filename, theta0, wave_theta, Part, AllSNcurve, Mthd8Ipt, unit,scale, RunType)
%Analysis Method 6:
...
periods = Mthd8Ipt.periods; %sec
Gamma1 = Mthd8Ipt.gamma1; 
Gamma2 = Mthd8Ipt.gamma2;
fr = 1./periods;  %1/sec
...
%     Gamma2 = 2.2;
    ...
    Tz = (SpectralMoment0./SpectralMoment2).^0.5;
    nu = (SQ./(2.*((2)^(0.5)).*(SpectralMoment0).^0.5)).^2; %BV 6.3.3 [NI 539]
    seastate3(:,i) = Prob(i)*Year_Length./Tz.*(1/A*((2.*(2*SpectralMoment0).^0.5).^m.*gammainc(m/2+1,nu,'upper'))+1/C*((2.*(2*SpectralMoment0).^0.5).^r.*gammainc(r/2+1,nu,'lower'))); % BV 3.4.2 [NI 611]
...
Life_method_BV = 1./(sum(seastate3,2)); % bilinear curve
...
if strcmp(RunType,'hsp')
    for p = 1:NoElem
        ...
        ElemRst{p}.Method = 6;
        ElemRst{p}.TotalLife = Life_method_BV(p);
    ...
    PartResultFile = [path_itr '\Results_Mthd8\' PartName '\ElemResult_' TorB '_Method8_Bin' num2str(NoBin) '.mat' ]; 
